...
public class Files {
     ...
     *
     * @param path      The path, possibly with an extension to strip
     * @param extension The extension to strip, or null if no extension exists
    ...
    public static String filename(final String path) {
        return Strings.lastPathComponent(path.replace('/', File.separatorChar),
                File.separatorChar);
    }
     ...
     * to {@link #remove(java.io.File)} for plain files.
     *
     ...
     */
    public static final int writeTo(final File file, final InputStream input)
            throws IOException {
     ...
     * @param file    The file to write to
     * @param input   The input
     * @param bufSize The memory buffer size. 4096 is a good value.
     * @return Number of bytes written
     * @throws java.io.IOException
     */
    public static final int writeTo(final File file, final InputStream input,
     ...
     * @param url url of the resource
     * @return reference to a local file if url contains one, <code>null</code> otherwise
     * @see #getLocalFileFromUrl(java.net.URL)
     */
